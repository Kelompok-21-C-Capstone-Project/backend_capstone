...
func (repo *PostgresRepository) UsersFindAll(uid string, params ...string) (dataCount int64, transactions *[]dto.ClientTransactionsResponse, err error) {
	...
	if params[1] != "" {
		if den == -1 {
			if err = repo.db.Debug().Order("transactions.created_at desc").Table("transactions").Select("transactions.id, product_categories.slug as category, product_categories.icon as icon, payments.status as status, products.name as product, transactions.description as transaction_details, payments.billed as charged, to_char(payments.created_at::timestamp at time zone 'gmt+7','DD-MM-YYYY HH24:MI:SS'), payments.method as payment_method").Joins("left join payments on payments.transaction_id = transactions.id").Joins("left join products on transactions.product_id = products.id").Joins("left join product_brand_categories on products.product_brand_category_id = product_brand_categories.id").Joins("left join product_categories on product_categories.id = product_brand_categories.product_category_id").Where("transactions.deleted is null and transactions.user_id = ? and lower(product_categories.slug) like lower(?) and lower(payments.status) like lower(?) and to_char(transactions.created_at::timestamp at time zone 'gmt+7','DD-MM-YYYY') = ? and (lower(transactions.id) like lower(?) or lower(products.name) like lower(?))", uid, params[4], params[3], params[1], params[0], params[0]).Count(&dataCount).Scan(&transactions).Error; err != nil {
				return
			}
			return
		}
		...
		if den == -1 {
			if err = repo.db.Debug().Order("transactions.created_at desc").Table("transactions").Select("transactions.id, product_categories.slug as category, product_categories.icon as icon, payments.status as status, products.name as product, transactions.description as transaction_details, payments.billed as charged, to_char(payments.created_at::timestamp at time zone 'gmt+7','DD-MM-YYYY HH24:MI:SS'), payments.method as payment_method").Joins("left join payments on payments.transaction_id = transactions.id").Joins("left join products on transactions.product_id = products.id").Joins("left join product_brand_categories on products.product_brand_category_id = product_brand_categories.id").Joins("left join product_categories on product_categories.id = product_brand_categories.product_category_id").Where("transactions.deleted is null and transactions.user_id = ? and lower(product_categories.slug) like lower(?) and lower(payments.status) like lower(?) and (lower(transactions.id) like lower(?) or lower(products.name) like lower(?)) and to_char(transactions.created_at::timestamp at time zone 'gmt+7','DD-MM-YYYY') >= ? and to_char(transactions.created_at::timestamp at time zone 'gmt+7','DD-MM-YYYY') <= ?", uid, params[4], params[3], params[0], params[0], params[7], params[8]).Count(&dataCount).Scan(&transactions).Error; err != nil {
				return
			}
			return
		}
...
func (repo *PostgresRepository) FindAll() (transactions *[]dto.ClientTransactionsResponse, err error) {
	if err = repo.db.Order("created_at desc").Table("transactions").Select("transactions.id, product_categories.slug as category, product_categories.icon as icon, payments.status as status, products.name as product, transactions.description as transaction_details, payments.billed as charged, to_char(payments.created_at::timestamp at time zone 'gmt+7','DD-MM-YYYY HH24:MI:SS'), payments.method as payment_method").Joins("left join payments on payments.transaction_id = transactions.id").Joins("left join products on transactions.product_id = products.id").Joins("left join product_brand_categories on products.product_brand_category_id = product_brand_categories.id").Joins("left join product_categories on product_categories.id = product_brand_categories.product_category_id").Scan(&transactions).Error; err != nil {
		return
	}
	return
}
